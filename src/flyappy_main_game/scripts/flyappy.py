...
PATHTOFLYAPPY = rospack.get_path('flyappy_main_game')
...
# list of backgrounds
BACKGROUNDS_LIST = (
    PATHTOFLYAPPY + '/assets/sprites/background-night.png',
)

# list of pipes
PIPES_LIST = (
    PATHTOFLYAPPY + '/assets/sprites/pipe-red.png',
)
...
def main():
    ...
    #define subscribers
    rospy.Subscriber("flyappy_acc", Vector3, controlCallback)
    #define shutdown
    ...
    # game over sprite
    IMAGES['gameover'] = pygame.image.load(PATHTOFLYAPPY + '/assets/sprites/gameover.png').convert_alpha()
    # message sprite for welcome screen
    IMAGES['message'] = pygame.image.load(PATHTOFLYAPPY + '/assets/sprites/message.png').convert_alpha()
    # base (ground) sprite
    IMAGES['base'] = pygame.image.load(PATHTOFLYAPPY + '/assets/sprites/base.png').convert_alpha()
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
            if event.type == KEYDOWN and (event.key == K_UP or event.key == K_DOWN or
        ...
        SCREEN.blit(IMAGES['base'], roundTuple((basex, BASEY)))
...
def mainGame(movementInfo):
    ...
    while True:
        ...
        for uPipe, lPipe in zip(upperPipes, lowerPipes):
            SCREEN.blit(IMAGES['pipe'][0], roundTuple((uPipe['x'], uPipe['y'])))
            SCREEN.blit(IMAGES['pipe'][1], roundTuple((lPipe['x'], lPipe['y'])))

        SCREEN.blit(IMAGES['base'], roundTuple((basex, BASEY)))
        ###################################################################
        # get bitmap of obstacles
        bitmap = getBitmap(upperPipes,lowerPipes,roundTuple((basex, BASEY)))
        ...
        SCREEN.blit(playerSurface, roundTuple((playerx, playery)))
...
def showScore(score):
    ...
    for digit in scoreDigits:
        SCREEN.blit(IMAGES['numbers'][digit], roundTuple((Xoffset, SCREENHEIGHT * 0.1)))
...
def showCounter(counter):
    ...
    for digit in scoreDigits:
        SCREEN.blit(IMAGES['numbers'][digit], roundTuple((Xoffset, SCREENHEIGHT * 0.85)))
        Xoffset += IMAGES['numbers'][digit].get_width()
...
def checkCrash(player, upperPipes, lowerPipes):
    ...
    if player['y'] + player['h'] >= BASEY - 1:
        ...
        playerRect = pygame.Rect(*roundTuple((player['x'], player['y'], player['w'], player['h'])))
        pipeW = IMAGES['pipe'][0].get_width()
        pipeH = IMAGES['pipe'][0].get_height()

        for uPipe, lPipe in zip(upperPipes, lowerPipes):
            # upper and lower pipe rects
            uPipeRect = pygame.Rect(*roundTuple((uPipe['x'], uPipe['y'], pipeW, pipeH)))
            lPipeRect = pygame.Rect(*roundTuple((lPipe['x'], lPipe['y'], pipeW, pipeH)))
...
def getBitmap(upperPipes,lowerPipes,base):
    ...
    #Add obstacles
    for uPipe, lPipe in zip(upperPipes, lowerPipes):
        obstacleSurface.blit(IMAGES['pipe'][0], roundTuple((uPipe['x'], uPipe['y'])))
        obstacleSurface.blit(IMAGES['pipe'][1], roundTuple((lPipe['x'], lPipe['y'])))
